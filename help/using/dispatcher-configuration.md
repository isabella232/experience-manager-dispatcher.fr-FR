...
* Lorsqu’il est défini sur « 1 », le nom d’hôte sera résolu à chaque requête (Dispatcher ne mettra jamais d’adresse IP en cache). Il peut y avoir un léger impact sur les performances en raison de l’appel supplémentaire nécessaire pour obtenir les informations d’hôte pour chaque requête.
...
* **Eléments avancés de la ligne de requête :** À compter de Dispatcher 4.2.0, quatre nouveaux éléments de filtre peuvent être utilisés. These new elements are `/path`, `/selectors`, `/extension` and `/suffix` respectively. Incluez un ou plusieurs de ces éléments pour mieux contrôler les schémas d&#39;URL.
...
>[!CAUTION]
>
>Le filtrage avec un agrégat est abandonné dans Répartiteur. As such, you should avoid using globs in the `/filter` sections since it may lead to security issues. Ainsi, au lieu de :
...
Après Dispatcher 4.2.0, vous pouvez inclure les expressions régulières POSIX Extended dans vos modèles de filtre.
...
* Les fichiers sont invalidés en touchant le fichier `.stat`. La date de dernière modification du fichier `.stat` est comparée à la date de dernière modification d’un document mis en cache. Le document est à nouveau récupéré si le fichier `.stat` est plus récent.
...
La catégorie `name` doit être unique à la ferme de serveurs. Le `pattern` est décrit dans la section [Conception de modèles pour les propriétés glob](#designing-patterns-for-glob-properties).
...
Pour ignorer ces interruptions, ajoutez le paramètre suivant `dispatcher.any` (before `/farms`) :
...
|--- |--- |--- |
| `*` | Correspond à aucune ou à plusieurs instances contiguës de n’importe quel caractère de la chaîne. Le dernier caractère de la correspondance est déterminé par l’une des situations suivantes :  <br/>Un caractère de la chaîne correspond au caractère suivant du modèle, et le caractère du modèle possède les caractéristiques suivantes :<br/><ul><li>N’est pas un *</li><li>N’est pas un ?</li><li>Un caractère littéral (incluant un espace) ou une classe de caractères.</li><li>La fin du modèle est atteinte.</li></ul>Dans une classe de caractères, le caractère est interprété littéralement. | `*/geo*`Correspond à n’importe quelle page sous les nœud `/content/geometrixx` et `/content/geometrixx-outdoors`. Les demandes HTTP suivantes correspondent au modèle glob : <br/><ul><li>`"GET /content/geometrixx/en.html"`</li><li>`"GET /content/geometrixx-outdoors/en.html"` </li></ul><br/> `*outdoors/*`<br/>Représente n’importe quelle page sous le nœud `/content/geometrixx-outdoors`. Par exemple, la requête HTTP suivante correspond au modèle glob : <br/><ul><li>`"GET /content/geometrixx-outdoors/en.html"`</li></ul> |
