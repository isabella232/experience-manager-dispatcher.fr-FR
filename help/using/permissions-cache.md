...
seo-title: Mise en cache du contenu sécurisé dans le répartiteur AEM
description: Découvrez comment la mise en cache sensible aux autorisations fonctionne dans Répartiteur.
seo-description: Découvrez comment la mise en cache sensible aux autorisations fonctionne dans le répartiteur AEM.
uuid: a abfed 68 a -2 efe -45 f 6-bdf 7-2284931629 d 6
...
La mise en cache sensible aux autorisations vous permet de mettre en cache des pages sécurisées. Le répartiteur vérifie les autorisations d&#39;accès d&#39;un utilisateur pour une page avant de diffuser la page mise en cache.
...
Le servlet doit être accessible à tous les utilisateurs. Par conséquent, la servlet doit étendre `org.apache.sling.api.servlets.SlingSafeMethodsServlet` la classe, qui fournit un accès en lecture seule au système.

Le servlet reçoit uniquement les demandes HEAD du rendu. Il suffit donc de mettre en œuvre la méthode `doHead`.

Le rendu inclut l’URI de la ressource demandée sous la forme d’un paramètre de requête HTTP. For example, an authorization servlet is accessed via `/bin/permissioncheck`. Pour effectuer une vérification de sécurité sur la page /content/geometrixx-outdoors/fr.html, la fonctionnalité de rendu inclut l’URL suivante dans la requête HTTP :
